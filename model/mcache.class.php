...
class mcache extends base_model {
	...
	*/
	public function read($cachename, $arg = NULL, $arg3 = FALSE, $arg4 = FALSE) {
		...
		if(empty($data)) {
			$data = $this->xupdate($cachename, $arg);
		}
		$this->vars[$key] = $data;
		return $data;
	}
	...
	public function xupdate($cachename, $arg) {
	...
	private function real_get($cachename, $arg) {
		$k = "cache_{$cachename}_$arg";
		return $this->kv->get($k);
	}
	
	private function real_delete($cachename, $arg) {
		$k = "cache_{$cachename}_$arg";
		$this->kv->delete($k);
		return TRUE;
	}
	
	private function real_set($cachename, $arg, $data) {
		$k = "cache_{$cachename}_$arg";
		$this->kv->set($k, $data);
		return TRUE;
	}
